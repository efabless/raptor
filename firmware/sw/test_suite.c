...
// GPIO test
// The testbench adds 7 to the lower 4 bits and feeds the sum to the upper 4 bits
#if TEST == 0x30
void test_30(){
  int magic = 6;
  // dbgio_startTest(0x30, 0xFFF);
  gpio_set_dir(240);   // lower 4 pins are o/p and higher 4 pins are i/p
  gpio_write(magic);
  // int data = gpio_read();
  // if(((data>>4) & 15) == (magic+7)) dbgio_endTest(1);
  // dbgio_endTest(0);
