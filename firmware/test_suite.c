...
// GPIO test
// The testbench adds 127 to the lower 8 bits and feeds the sum to the upper 8 bits
#if TEST == 0x30
void test_30(){
  int magic = 6;
  dbgio_startTest(0x30, 0xFFF);
  gpio_set_dir(65280);   // lower 8 pins are o/p and higher 8 pins are i/p
  gpio_write(magic);
  int data = gpio_read();
  if(((data>>8) & 255) == (magic+127)) dbgio_endTest(1);
  dbgio_endTest(0);
